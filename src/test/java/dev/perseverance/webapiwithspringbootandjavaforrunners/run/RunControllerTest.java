...
class RunControllerTest {
    ...
    void shouldFindOneRun() throws Exception {
        ...
        mvc.perform(get("/api/runs/1"))
                .andExpect(status().isOk())
                .andExpect((ResultMatcher) jsonPath("$.id", is(run.id())))
                .andExpect((ResultMatcher) jsonPath("$.title", is(run.title())))
                .andExpect((ResultMatcher) jsonPath("$.miles", is(run.miles())))
                .andExpect((ResultMatcher) jsonPath("$.location", is(run.location().toString())));
    }
